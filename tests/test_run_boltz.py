"""Tests for scripts/run_boltz.py pip-install probing."""
...
import scripts.run_boltz as boltz
...
def test_pip_probe_uses_sys_executable_and_survives_timeout(monkeypatch):
    ...
    timeout (importing boltz pulls in torch) as a failed discovery path, not
    a crash."""
    calls = []
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd[0] == sys.executable and "boltz" in cmd:
    ...
    assert boltz.find_boltz({}) is None

    pip_probes = [c for c in calls if c[0] == sys.executable and "boltz" in c]
